...
export function QuizIntro({ copy, started, onStart }: QuizIntroProps) {
  ...
  return (
    <div className="flex flex-1 flex-col pt-4 sm:pt-8">
      <motion.h1
        {...enter(0)}
      ...
      <motion.p {...enter(0.08)} className="mt-3 text-sm text-pretty text-ink-muted sm:text-base">
        {copy.description}
      </motion.p>

      <motion.ul {...enter(0.16)} className="mt-6 flex flex-col gap-2.5" aria-label="O que você pode esperar">
        {TRUST.map((item) => (
          <li key={item.label} className="flex items-center gap-2.5 text-sm text-ink">
            <span className="grid size-7 shrink-0 place-items-center rounded-full border border-brand/30 bg-brand-dim/50 text-brand-ink">
