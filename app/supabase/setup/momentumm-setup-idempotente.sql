...
-- ---------------------------------------------------------------------------
-- 0007_plan_stages.sql — a etapa, o degrau entre objetivo e ação.
--
-- Aditivo como o resto: nenhuma coluna é removida, nenhuma linha é reescrita.
-- Ação sem etapa continua válida (é o objetivo que ainda não virou plano) e
-- conta exatamente como contava antes.
-- ---------------------------------------------------------------------------
...
-- 'atrasada' NÃO é valor guardado: ela é derivada de `due_on` contra o dia de
-- hoje. Guardar abriria a porta pra uma etapa marcada atrasada com prazo lá na
-- frente — a mesma decisão já tomada em objectives (estado x status).

create table if not exists public.plan_stages (
  id           uuid primary key default gen_random_uuid(),
  user_id      uuid not null references public.profiles (id) on delete cascade,
  ...
  -- de tabela: uma checagem por linha não vê o conjunto, e uma constraint
  -- deferida travaria a reordenação no meio.
...
create index if not exists plan_stages_objective_idx
  on public.plan_stages (objective_id, sort_order);

create index if not exists plan_stages_user_idx on public.plan_stages (user_id);
...
alter table public.tasks
  ...
  add column if not exists stage_id    uuid references public.plan_stages (id) on delete set null,
  add column if not exists weight      smallint not null default 1 check (weight between 1 and 10),
...
create index if not exists tasks_stage_idx on public.tasks (stage_id, sort_order)
...
create index if not exists habits_stage_idx on public.habits (stage_id) where stage_id is not null;
...
-- só passa a andar errado. O domínio já recusa; o trigger garante o mesmo pra
-- qualquer caminho que não passe pelo app.
-- ---------------------------------------------------------------------------
...
drop trigger if exists tasks_stage_matches_objective on public.tasks;
...
drop trigger if exists habits_stage_matches_objective on public.habits;
create trigger habits_stage_matches_objective
  before insert or update of stage_id, objective_id on public.habits
  for each row execute function public.assert_stage_matches_objective();

-- ---------------------------------------------------------------------------
-- Integridade: concluída tem data de conclusão.
--
...
-- é uma correção óbvia e segura, e recusar faria a pessoa perder o clique de
-- concluir por causa de um detalhe que o banco sabe resolver.
-- ---------------------------------------------------------------------------
...
drop trigger if exists plan_stages_stamp_completion on public.plan_stages;
