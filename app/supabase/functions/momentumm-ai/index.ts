...
Deno.serve(async (request) => {
  ...
  const limit = PLAN_LIMITS[tier].aiCallsPerDay
  ...
  if (used >= limit) {
    return fail(
      ...
      'quota_exceeded',
      tier === 'pro'
        ? `Você já usou as ${limit} leituras de hoje. Amanhã libera de novo.`
        : `O plano gratuito tem ${limit} leituras da IA por dia, e as de hoje já foram. No PRO são ${PLAN_LIMITS.pro.aiCallsPerDay}.`,
    )
  }
